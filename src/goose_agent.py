...
class BlockGooseAgent(CodeAgent):
    """Agent that interfaces with Block Goose to generate and run code"""
    
    def __init__(self, config: VerifierConfig, agent_type: str):
        super().__init__(config, agent_type)
        self._verify_goose_installation()
        
    def _verify_goose_installation(self):
        """Verify that Block Goose is installed and correctly configured"""
        try:
            ...
            if result.returncode != 0:
                raise RuntimeError(f"Block Goose command failed: {result.stderr}")
                
            print(f"Block Goose is installed: {result.stdout.strip()}")
            
        except FileNotFoundError:
            raise RuntimeError(
                "Block Goose is not installed. Please install it using:\n"
                "curl -fsSL https://github.com/block/goose/releases/download/stable/download_cli.sh | bash"
            )
        except subprocess.TimeoutExpired:
            raise RuntimeError("Block Goose installation check timed out")
        except Exception as e:
            raise RuntimeError(f"Failed to verify Block Goose installation: {e}")
...
"""
    ...
    def _get_log_file_path(self) -> Path:
        ...
        return Path(self.config.goose_log_file)
        
    def _get_session_start_message(self) -> str:
        """Get the session start message for this agent type"""
        return f"Block Goose {self.agent_type} agent session started. Mission: {self.config.agent_mission}"
        
    def _get_process_success_message(self, change_count: int) -> str:
        """Get the success message after processing changes"""
        return f"Block Goose processed {change_count} file changes"
        
    async def _run_code_tool(self, prompt: str) -> str:
        """Run Block Goose with the given prompt"""
        response = ""
        error = None
        success = False
        
        try:
            # Create a temporary directory for the goose session
            import tempfile
            import os
            
            with tempfile.TemporaryDirectory() as temp_dir:
                # Create a temporary file for the prompt
                temp_prompt_file = Path(temp_dir) / "prompt.txt"
                temp_prompt_file.write_text(prompt)
                
                # Run goose session with stdin input
                # Block Goose works best with interactive sessions, so we'll
                # pipe the prompt to stdin and capture the output
                ...
                # Send the prompt to goose and then send exit command
                input_text = f"{prompt}\n\nexit\n"
                stdout, stderr = await asyncio.wait_for(
                    process.communicate(input=input_text.encode()),
                    timeout=self.config.goose_timeout
                )
                
                if process.returncode != 0:
                    error = f"Block Goose failed: {stderr.decode()}"
            ...
            error = f"Block Goose timed out after {self.config.goose_timeout}s"
            raise RuntimeError(error)
        except Exception as e:
            error = f"Failed to run Block Goose: {e}"
            raise RuntimeError(error)
        finally:
...
class BlockGooseVerifierAgent(BlockGooseAgent):
    """Block Goose agent specialized for verification tasks"""
...
class BlockGooseDocumentationAgent(BlockGooseAgent):
    """Block Goose agent specialized for documentation tasks"""
